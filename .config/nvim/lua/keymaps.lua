-- Non plugin 
vim.g.mapleader = " "
vim.keymap.set("n", "<leader>bt", '<cmd>lua TransparentColor()<CR>', {desc = "Transparent background"})
vim.keymap.set("n", "<leader>bf", '<cmd>lua FocusedColor()<CR>', {desc = "Solid background"})
...
vim.keymap.set("n", "<leader>h", function() harpoon:list():select(1) end, {desc = "Got to first file in list"})
vim.keymap.set("n", "<leader>j", function() harpoon:list():select(2) end, {desc = "Got to second file in list"})
vim.keymap.set("n", "<leader>k", function() harpoon:list():select(3) end, {desc = "Got to third file in list"})
vim.keymap.set("n", "<leader>l", function() harpoon:list():select(4) end, {desc = "Got to fourth file in list"})
-- Toggle previous & next buffers stored within Harpoon list
vim.keymap.set("n", "<C-S-P>", function() harpoon:list():prev() end, {desc = "Go to previous file in list"})
vim.keymap.set("n", "<C-S-N>", function() harpoon:list():next() end, {desc = "Go to next file in list"})


-- Trouble
vim.keymap.set("n", "<leader>to", function() require("trouble").open() end, {desc="Open trouble"})
vim.keymap.set("n", "<leader>tw", function() require("trouble").open("workspace_diagnostics") end, {desc="Open workspace diagnostics"})
vim.keymap.set("n", "<leader>td", function() require("trouble").open("document_diagnostics") end, {desc="Open document diagnostics"})
vim.keymap.set("n", "<leader>tr", '<cmd>TroubleRefresh<CR>', {desc="Refresh trouble"})
vim.keymap.set("n", "<leader>tlr", function() require("trouble").open("lsp_references") end, {desc="Open lsp references"})
vim.keymap.set("n", "<leader>tld", function() require("trouble").open("lsp_definitions") end, {desc="Open lsp definitions"})
vim.keymap.set("n", "<leader>tlt", function() require("trouble").open("lsp_type_definitions") end, {desc="Open lsp type definitions"})
